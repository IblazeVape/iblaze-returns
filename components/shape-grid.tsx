...
/** Animated grid-of-shapes canvas background — a subtle, interactive
 * texture for otherwise-empty panels (e.g. the guest lookup screen).
 * Colors default to a light theme (light borders/hover) to sit on a white
 * background, unlike the dark-theme demo this was adapted from. */
export function ShapeGrid({
  direction = "diagonal",
  speed = 0.5,
  borderColor = "#e4e4e7",
  squareSize = 40,
  hoverFillColor = "rgba(0, 0, 0, 0.035)",
  shape = "square",
  hoverTrailAmount = 0,
  className,
}: ShapeGridProps) {
  ...
  useEffect(() => {
    ...
    const cellAt = (mouseX: number, mouseY: number): GridOffset => {
      if (isHex) {
        const colShift = Math.floor(gridOffset.current.x / hexHoriz)
        const offsetX = ((gridOffset.current.x % hexHoriz) + hexHoriz) % hexHoriz
        const offsetY = ((gridOffset.current.y % hexVert) + hexVert) % hexVert
      ...
      if (isTri) {
        ...
        const offsetX = ((gridOffset.current.x % halfW) + halfW) % halfW
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize
        return { x: Math.round((mouseX - offsetX) / halfW), y: Math.floor((mouseY - offsetY) / squareSize) }
      }
      if (shape === "circle") {
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize
        return { x: Math.round((mouseX - offsetX) / squareSize), y: Math.round((mouseY - offsetY) / squareSize) }
      }
      const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize
      const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize
      return { x: Math.floor((mouseX - offsetX) / squareSize), y: Math.floor((mouseY - offsetY) / squareSize) }
    }

    const handleMouseMove = (event: MouseEvent) => {
      const rect = canvas.getBoundingClientRect()
      const cell = cellAt(event.clientX - rect.left, event.clientY - rect.top)
      if (!hoveredSquareRef.current || hoveredSquareRef.current.x !== cell.x || hoveredSquareRef.current.y !== cell.y) {
