...
/** Interactive dot-grid background — dots bulge away from the cursor with a
 * soft radial glow trailing it. Adapted from a component the user provided;
 * colors default to a light theme to sit on the portal's white background. */
    ...
    function tick() {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            const bt = 1 - dist / cr
            const push = bt * bt * (p.bulgeStrength as number) * eng
