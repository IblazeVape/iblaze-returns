...
// Clean shadcn-style navbar block for the page builder. Everything is
// editable from the editor: logo (image URL or the built-in mark), menu
// links, and both buttons. Added per the "every block fully editable" rule.
...
const DEFAULT_LINKS: SimpleNavLink[] = [
  { title: "Home", href: "/" },
  { title: "Blog", href: "#" },
  { title: "About", href: "#" },
...
export function SimpleNavLogo({ logoUrl, brand }: { logoUrl?: string; brand?: string }) {
  if (logoUrl) {
    return (
  ...
  return (
    <span className="flex items-center gap-2">
      <DefaultMark />
      {brand && <span className="text-xl font-semibold tracking-tight">{brand}</span>}
    </span>
  )
}

function Links({ links, orientation }: { links: SimpleNavLink[]; orientation?: "horizontal" | "vertical" }) {
  return (
    <NavigationMenu orientation={orientation} className={orientation === "vertical" ? "mt-6 [&>div]:h-full" : undefined}>
      <NavigationMenuList className="data-[orientation=vertical]:-ms-2 data-[orientation=vertical]:flex-col data-[orientation=vertical]:items-start data-[orientation=vertical]:justify-start">
        {links.map((l) => (
          <NavigationMenuItem key={l.title + l.href}>
...
export function SimpleNav({
  ...
  return (
    <nav className="h-16 border-b bg-background text-foreground">
          ...
          <div className="md:hidden">
            <Sheet>
              ...
              <SheetContent className="px-6 py-3">
                ...
                <SimpleNavLogo logoUrl={logoUrl} brand={brand} />
                <Links links={links} orientation="vertical" />
              </SheetContent>
            </Sheet>
          </div>
        </div>
...
// The stock logomark used until a logo image URL is set.
